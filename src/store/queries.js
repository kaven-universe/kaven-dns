...
/**
 * Query history (ring buffer, drops oldest when full) + aggregate stats.
 ...
 * pressure (see enforceMemoryBound()), so the history can grow as large as
...
class QueryStore {
  ...
  load() {
    try {
      ...
      if (e.code !== 'ENOENT') console.error(`[queries] Failed to read ${this.file}: ${e.message}`);
    }
  }
