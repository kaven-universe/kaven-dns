...
- **Bilingual Web console** (Chinese / English, simple password authentication):
  ...
  - Settings: a responsive, equal-height card grid for the upstream list, cache and query-retention parameters, ports, password change, cache flush, query-history reset, stable-version update check, and display preferences
  - Browser-local display preferences: each query table has its own outside-click/Escape-dismissable column picker; timestamps support date-and-time, date-only, time-only, local 24-hour / 12-hour, UTC ISO 8601, and custom token-based formats; preferences apply immediately and do not alter server configuration
